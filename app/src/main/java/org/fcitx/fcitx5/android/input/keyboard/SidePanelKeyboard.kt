...
abstract class SidePanelKeyboard(
    context: Context,
    theme: Theme,
    ...
    init {
        ...
        post {
            sideLayoutKeyView.setOnItemActionListener { action ->
                this@SidePanelKeyboard.onAction(action)
            }
    ...
    private fun updateSideLayoutHeight() {
        ...
        sideLayoutKeyView.updateVisibleItemCount(sideLayoutKeyColumnShowNum)
    }
    ...
    protected fun handleFcitxEvent(event: FcitxEvent<*>) {
        when (event) {
            ...
            is FcitxEvent.InputPanelEvent -> {
                if (!shouldUpdateSidePanelWhenFcitxEvent()) {
                    return
                }
    ...
    fun updateSideBarItems(newItems: List<KeyDef>) {
        ...
        sideLayoutKeyView.updateItems(newItems)
    }
    ...
    abstract fun shouldUpdateSidePanelWhenFcitxEvent(): Boolean
